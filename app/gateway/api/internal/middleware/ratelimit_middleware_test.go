...
func TestRateLimitMiddleware_AllowsWithinBudget(t *testing.T) {
	...
	fire, calls := recorderAndHandler(t, store, NewRateLimitMiddleware(store).Handle)
...
func TestRateLimitMiddleware_BucketIsolation(t *testing.T) {
	store, _ := newRedisStore(t, "aim:gateway:quota", time.Minute, 1)
	fire, _ := recorderAndHandler(t, store, NewRateLimitMiddleware(store).Handle)
...
func TestRateLimitMiddleware_NilStorePassesThrough(t *testing.T) {
	fire, calls := recorderAndHandler(t, nil, NewRateLimitMiddleware(nil).Handle)
	req := httptest.NewRequest(http.MethodGet, "/x", nil)
...
func TestRateLimitMiddleware_MissingIdentityPassesThrough(t *testing.T) {
	store, _ := newRedisStore(t, "aim:gateway:quota", time.Minute, 1)
	fire, calls := recorderAndHandler(t, store, NewRateLimitMiddleware(store).Handle)
...
func TestRateLimitMiddleware_RedisKeyShape(t *testing.T) {
...
// objects (different stores, different middleware types).
func TestRateLimitMiddlewares_AreIndependent(t *testing.T) {
	store, _ := newRedisStore(t, "aim:gateway:quota", time.Minute, 5)
	human := NewRateLimitMiddleware(store)
