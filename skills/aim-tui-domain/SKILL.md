---
name: aim-tui-domain
description: aim 的客户端域。包括 `app/tui`（TUI）与 `app/desktop`（Wails 桌面端）两个客户端。
---
# aim-tui-domain

## 使用范围

当需求涉及 `app/tui`（TUI 终端客户端）或 `app/desktop`（Wails 桌面客户端）、CLI 参数、Bubble Tea 布局、本地 SQLite 缓存、WebSocket/REST 客户端、token 自动刷新、本机多实例/多账号隔离时使用本 Skill。

## 模块边界

- TUI / Desktop 只和 gateway 通信。
- TUI / Desktop 不直接导入 auth/core/logic 的内部包。
- 本地状态只作为客户端缓存，不作为服务端权威状态。
- Desktop 支持同设备多账号：每个账号拥有独立 `device_id`/token 与 SQLite 缓存。

## 参考资料

- `references/gateway-api-reference.md` — Gateway REST + WebSocket 帧协议（客户端对接必读）
- `references/runtime-design.md`
- `references/local-store.md`
- `references/desktop-multi-account.md` — Desktop 多账号登录与缓存隔离设计
