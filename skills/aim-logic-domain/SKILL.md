...
- 2026-05-29: 修复统一搜索 gRPC 注册。`app/logic/rpc/logic.go` 现在注册 `SearchServiceServer`，避免 gateway `GET /api/search` 调用 `UnifiedSearch` 时收到 gRPC `Unimplemented`。新增注册回归测试覆盖 `logic.SearchService`。
