...
- 2026-05-24: WebSocket upgrade 后的 per-connection context 使用 `shared/tracing.DetachSpanContext` 清除 HTTP upgrade span 作为当前 span，避免长连接期间 per-frame/RPC/Kafka 短 span 引用尚未结束的 upgrade span，避免长连接 upgrade span 成为短生命周期子 span 的父级，减少 tracing backend 的无效父 span/时钟偏移类告警。
